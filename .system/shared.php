...
/**
 * Where we avoid some duplication.
 *
 */
...
function rssTitle($s_string)
{
    ...
    $s_string = str_replace(
        array('<strong>', '</strong>', '<em>', '</em>', '<q>', '</q>', '<code>', '</code>', '<br />'), array('*', '*', '_', '_', '“', '”', '`', '`', ' '), $s_string
    );
...
function indexSite()
{
    ...
    foreach ($types as $type => &$count)
        foreach (preg_grep('/\.rem$/', scandir($type)) as $file_name) {
            $name = pathinfo($file_name, PATHINFO_FILENAME);

            //get the article’s header and content. if it doesn’t have a header, skip it
            if (!list ($meta, $title, $content) = getArticle("$type/$name"))
                continue;
            ...
            if ($modified) {
                ...
                if (!is_writable("$type/$file_name"))
                    @chmod("$type/$file_name", 0777) or errorPage(
                        'denied_article.rem', 'Error: Permission Denied', array('PATH' => "$type/$file_name")
                    );
                file_put_contents("$type/$file_name", "{\t\"date\"\t\t:\t${meta['date']},\n"
            ...
            array_push($list, $meta['updated'] . "|$type|" . @implode('|', $meta['tags']) . (@$meta['tags'] ? '|' : '') . $name
            );
    ...
    /* save list to cache
      -------------------------------------------------------------------------------------------------------------- */
    if (!is_writable(APP_CACHE))
        @chmod(APP_CACHE, 0777) or errorPage(
...
function templateHeader($s_href = '', $s_next = '', $s_prev = '', $s_canonical = '')
{
    ...
    if (file_exists(APP_CACHE . 'index.list')) {
        ...
        foreach (getTypes() as $category => $count)
            $types .= template_tags($tag, array(
                'TAG'  => $category,
                'ON'   => $filter == $category ? $on : '',
                'HREF' => "/$category/"
            ));
        foreach (getTags() as $category => $count)
            $tags .= template_tags($tag, array(
                'TAG'  => $category,
                'ON'   => $filter == $category ? $on : '',
                'HREF' => "/$category/"
            ));
        ...
        return template_tags(template_load('base.header.html'), array(
            ...
            'ALL_ON'      => !$filter && $article !== 'imprint' && $article !== 'projects' ? $on : '', //home page
            'IMPRINT_ON'  => $article == 'imprint' ? $on : '', //imprint page
            'PROJECTS_ON' => $article == 'projects' ? $on : '', //projects page
            'CLOUD_TYPES' => $types,
            'CLOUD_TAGS'  => $tags, //type/tag cloud
            'PREV'        => $s_prev,
            'NEXT'        => $s_next, //navigation links
        ));
    }
}
...
function templateFooter($s_href)
{
    return template_tags(template_load('base.footer.html'), array(
        'HREF'           => $s_href,
        'HOST'           => APP_HOST,
        'SITE_ANALYTICS' => (DEVELOPMENT === false ? template_tag(template_load('base.footer.analytics.html'), 'ANALYTICS_KEY', ANALYTICS_KEY) : ''),
    ));
}
...
function templateArticle(&$a_meta, $type, $href, &$s_content, $category)
{
    ...
    /* an image enclosure gets a preview image
      -------------------------------------------------------------------------------------------------------------- */
    if (@$m_enclosure)
        foreach ($m_enclosure as $enclosure)
            switch (true) {
                ...
                case list ($preview_width, $preview_height, $preview_type) = getimagesize(APP_ROOT . "$href/$enclosure"):
                    ...
                    $is_alpha = ($preview_type == IMAGETYPE_PNG) ? ord(file_get_contents(APP_ROOT . "$href/$enclosure", false, null, 25, 1)) & 4 : false;
                    ...
                    $preview_file = $preview_width <= APP_PREVIEW_SIZE ? $enclosure : pathinfo($enclosure, PATHINFO_FILENAME) . '_preview.' . ($is_alpha ? 'png' : 'jpg');
                    //scale the height according to ratio
                    $preview_height = $preview_width <= APP_PREVIEW_SIZE ? $preview_height : APP_PREVIEW_SIZE * ($preview_height / $preview_width);
                    $preview_width  = $preview_width <= APP_PREVIEW_SIZE ? $preview_width : APP_PREVIEW_SIZE;
                    ...
                    if ( /*  a preview file is required, and it does not exist, create it...
                      ------------------------------------------------------------------------------------------------- */
                        ...
                        switch ($preview_type) {
                            case IMAGETYPE_JPEG:
                                $image = imagecreatefromjpeg(APP_ROOT . "$href/$enclosure");
                                break;
                            case IMAGETYPE_PNG:
                                $image = imagecreatefrompng(APP_ROOT . "$href/$enclosure");
                                break;
                        }
                        //resize the image
                        imagecopyresampled(
                            $image_preview, $image, 0, 0, 0, 0, $preview_width, $preview_height, imagesx($image), imagesy($image)
                        );
                        imagedestroy($image);
                ...
                default:
                    @$enclosure_html .= template_tags(template_load('article.enclosure.html'), array(
                        ...
                        'SIZE' => array_reduce(array(' B', ' KB', ' MB'), create_function('$a,$b', 'return is_numeric($a)?($a>=1024?$a/1024:number_format($a,strlen($b)-2).$b):$a;'
    ...
    return template_tags(template_load('article.html'), array(
        ...
        //template each tag and concenate into a list
            $m_tags, create_function('$a,$b', 'return $a.=template_tags("' . addslashes(template_load('tag.html')) . '",' .
...
function errorPage($s_template, $s_title, $a_data = array())
{
    ...
    die(templatePage(
        reMarkable(template_tags(template_load("errors/$s_template"), $a_data)), $s_title, templateHeader(), templateFooter(".system/design/templates/errors/" . pathinfo($s_template, PATHINFO_FILENAME))
    ));
}
...
function errorPageHTTP($i_code)
{
    ...
    $html = templatePage(
        reMarkable(template_load("errors/$i_code.rem")), $i_code, templateHeader(), templateFooter(".system/design/templates/errors/$i_code")
    );
    //cache it
